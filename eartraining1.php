...
if (isset($_SESSION['MM_Username'])) {
  $colname_rsGame7 = $_SESSION['MM_Username'];
}
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsGame7=sprintf("SELECT game7responses.entryNumber, game7responses.questionNumber FROM game7responses WHERE userName=%s AND game7responses.tally=1 ORDER BY game7responses.questionNumber DESC",
  GetSQLValueString($colname_rsGame7, "text")); 
$rsGame7 = mysql_query($query_rsGame7, $ohiofi) or die(mysql_error());
$row_rsGame7 = mysql_fetch_assoc($rsGame7);
$totalRows_rsGame7 = mysql_num_rows($rsGame7);




mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScore3=sprintf("SELECT game7responses.entryNumber, game7responses.questionNumber FROM game7responses WHERE userName=%s",
  GetSQLValueString($colname_rsGame7, "text")); 
...
/* ------------------------------------- rsGame7_check is the fail safe -----------------------------------------*/
$totalRows_rsGame7_check = 0;
if (isset($_POST['questionNumber'])) {
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsGame7_check=sprintf("SELECT game7responses.entryNumber, game7responses.questionNumber FROM game7responses WHERE userName=%s AND game7responses.questionNumber=%s",
  GetSQLValueString($colname_rsGame7, "text"), GetSQLValueString($_POST['questionNumber'], "int")); 
$rsGame7_check = mysql_query($query_rsGame7_check, $ohiofi) or die(mysql_error());
$row_rsGame7_check = mysql_fetch_assoc($rsGame7_check);
$totalRows_rsGame7_check = mysql_num_rows($rsGame7_check);
}

mysql_select_db($database_ohiofi, $ohiofi);
$query_rsUser=sprintf("SELECT users.userID, users.game7 FROM users WHERE userName=%s ",
  GetSQLValueString($colname_rsGame7, "text")); 
...
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScoreboard = "SELECT userName, game7 FROM users ORDER BY game7 DESC";
...
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScoreboard = "SELECT userName, game7 FROM users WHERE game7 > 0 ORDER BY game7 DESC";
$query_limit_rsScoreboard = sprintf("%s LIMIT %d, %d", $query_rsScoreboard, $startRow_rsScoreboard, $maxRows_rsScoreboard);
$rsScoreboard = mysql_query($query_limit_rsScoreboard, $ohiofi) or die(mysql_error());
$row_rsScoreboard = mysql_fetch_assoc($rsScoreboard);
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	if ($totalRows_rsGame7_check == 0) {
		  $updateSQL = sprintf("UPDATE users SET game7=%s, game7total=%s WHERE userID=%s",
							   GetSQLValueString($_POST['points'], "int"),
							   GetSQLValueString($_POST['game7total'], "int"),
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
	if ($totalRows_rsGame7_check == 0) {
		  $updateSQL = sprintf("UPDATE users SET game7total=%s WHERE userID=%s",
							   GetSQLValueString($_POST['game7total'], "int"),
							   GetSQLValueString($_POST['userNumber'], "int"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	if ($totalRows_rsGame7_check == 0) {
		///if ($row_rsScore3['questionNumber'] == $totalRows_rsScore3){
	  		...
	  		$insertSQL = sprintf("INSERT INTO game7responses (entryNumber, userName, questionNumber, question, answer, tally) VALUES (%s, %s, %s, %s, %s, %s)",
...
<title>Ear Training 1 web app</title>
...
<body onload="preloader(7);" />
	...
	<div id="resistorsmain">  
	    <div id="stage">
    		<div id="head1">
            Level 1, Question <?=$currentQuestion?>
            <br />
    			<div class="topPlayButton" onclick="play();">PLAY</div>
    		</div>
            <br />
    		<div class="divider">Tones</div>
    		<div class="playButton floatLeft" onclick="gimme('250hz');" onmouseover="preloader(0);">250 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('400hz');" onmouseover="preloader(1);">400 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('600hz');" onmouseover="preloader(2);">600 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('1000hz');" onmouseover="preloader(3);">1000 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('1600hz');" onmouseover="preloader(4);">1600 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('2400hz');" onmouseover="preloader(5);">2400 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('4000hz');" onmouseover="preloader(6);">4000 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('6400hz');" onmouseover="preloader(7);">6400 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('9600hz');" onmouseover="preloader(8);">9600 Hz</div>
            <br />
    		<div class="divider">White Noise</div>
    		<div class="playButton floatLeft" onclick="gimme('whitenoise20hzto20000hz');" onmouseover="preloader(9);">20 Hz - 20000 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('whitenoise150hzto300hz');" onmouseover="preloader(10);">150 Hz - 300 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('whitenoise600hzto1200hz');" onmouseover="preloader(11);">600 Hz - 1200 Hz</div>
            <div class="playButton floatLeft" onclick="gimme('whitenoise2400hzto4800hz');" onmouseover="preloader(12);">2400 Hz - 4800 Hz</div>
    		<div class="playButton floatLeft" onclick="gimme('whitenoise9600hzto19200hz');" onmouseover="preloader(13);">9600 Hz - 19200 Hz</div>
	...
	<div id="tryAgain" class="popup">
            	<h2>Incorrect</h2>
       		  <form id="form2" name="form2" action="<?php echo $editFormAction; ?>" method="POST"><input name="entryNumber" type="hidden" value="" /><input name="userName" type="hidden" value="" /><input type="hidden" name="question2" id="question2" value=""><input type="hidden" name="response2" id="response2" value=""><input name="tally" type="hidden" value="0" /><input name="questionNumber" type="hidden" value="<?php echo $currentQuestion; ?>" /><input name="userNumber" type="hidden" value="<?=$row_rsUser["userID"] ?>" /><input name="game7total" type="hidden" value="<?php echo $totalRows_rsScore3; ?>" /><input class="submit button popupbutton" type="submit" name="Submit" id="Submit" value="Continue">
                ...
                <? if ($currentQuestion % 1 == 0) { ?>
                	...
                	<h2>Your current score is<br /><? echo $row_rsUser["game7"] ?> <? if ($row_rsUser["game7"] != 1)
                                echo " pts";
                            else
                                echo " pt";
                            ?></h2>
                      ...
                      <table border="0" STYLE="margin:15px;">
                        ...
                        do { ?>
                          <tr>
                            ...
                            <td><?php echo " "; ?></td>
                            <td><?php echo " ",$row_rsScoreboard['game7'];
                            if ($row_rsScoreboard['game7'] != 1)
                                echo " pts";
                            else
                                echo " pt";
                            ?>
...
var oneChance = 0;
var youReady = 0;
var randomMP3=new Array("250hz","400hz","600hz","1000hz","1600hz","2400hz","4000hz","6400hz","9600hz",'whitenoise20hzto20000hz','whitenoise150hzto300hz','whitenoise600hzto1200hz','whitenoise2400hzto4800hz','whitenoise9600hzto19200hz');
var question=randomMP3[7];

function newQuestion() {
	var oldQuestion=question;
	question=randomMP3[(Math.floor ( ( Math.random ( ) * 14) ))];
